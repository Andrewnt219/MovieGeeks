...
async function getAllGenres() {
  const respond = await fetch(
    `https://api.themoviedb.org/3/genre/movie/list?api_key=${process.env.API_KEY}&language=en-US&page=1&region=CA`
  );
  return respond.json();
}
...
async function populateMovies(movies) {
  ...
  movies.forEach((mov) => {
    ...
    for (m_genre of mov.genre_ids) {
      for (genre of genres) {
        if (m_genre === genre.id) mov.genres.push(genre.name);
      }
    }
  ...
  movies.forEach((mov) => {
    ...
    mov.poster_path
      ? (mov.poster_path =
          'http://image.tmdb.org/t/p/original' + mov.poster_path)
      : (mov.poster_path = '404.png');
...
async function populateMovie(movie) {
  ...
  movie.poster_path
    ...
    : (movie.poster_path = '404.png');

  movie.vote_average = movie.vote_average.toFixed(1);

  movie.price = (Math.round(Math.random() * (15 - 4 + 1) + 4) + 0.99).toFixed(
    2
  );
  movie.rent = Math.ceil(movie.price * 0.2);
  return movie;
}

async function movies(api) {
  const respond = await fetch(
    `https://api.themoviedb.org/3/movie/${api}?api_key=${process.env.API_KEY}&language=en-US&page=1&region=CA`
  );
...
async function discoverGenre(genre) {
  ...
  let respond = await fetch(
    `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=${genreId}`
  );
