...
function formatMovie(movie) {
  if (movie) {
    ...
    if (
      ...
      !movie.poster_path.includes(imgPath)
    )
    ...
    if (movie.price == 0) movie.price = 'N/A';
  }
...
router.get('/search', async (req, res) => {
  ...
  if (!mov)
    return res
      .status(404)
      .render('movie/movie', { title: '404: Movie not found' });
...
router.post('/add', async (req, res, next) => {
  ...
  for (const field in req.body) {
    if (String(field).match(/genre.*/)) movie.genre.push(req.body[field]);
  }
...
router.put('/edit/', async (req, res) => {
  ...
  for (field in req.body) {
    if (field.match(/genre.*/)) genres.push(req.body[field]);
  }
...
router.delete('/delete/:movieId', async (req, res) => {
  ...
  if (!movie)
    return res.render('utils/error', {
      title: 'Sorry',
      message: 'Movie not found',
    });

  res.render('utils/error', {
    message: `${movie.title} #${movie.id}`,
